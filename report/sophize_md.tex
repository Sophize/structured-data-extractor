...
In addition, we have developed an online tool that helps in extracting definitions, propositions and proofs from existing documents. In addition, it allows users to easily add semantic information to documents.
...
For ease of use, all the features of the tool have been integrated in a single page. The top section of the page has button for the loading, unloading, saving and creating new resources. A navigation bar on the left lets users easily switch between different resources currently loaded. Any changes made are automatically written to browser's local storage before the user chooses to save their changes to the external server.

In addition, we created a new context menu when user has selected a portion of text in any of the forms. The context menu has options to extract the selected text as a definition, proposition or argument and to convert the selet text to a Sophize Markdown link. Thus, by simply selecting and right-clicking the text we can perform the task of embedding and extracting semantic data in documents. The various options of these tasks are displayed using convenient modal dialogs.

\subsection{Server to convert \LaTeX\space document to Markdown }

Since most mathematical knowledge is present in \LaTeX\space documents, there is a need for an easy way to first convert these documents to Markdown before this tool can be used on them. Pandoc is the obvious choice for this conversion. A Python server that utilizes the Pandoc converter has been developed. It also performs some processing of the input and output to circumvent some limitations of Katex library that is used for rendering Math mode content. Using this server we provide a convenient copy-paste interface in the tool for easy conversion.
